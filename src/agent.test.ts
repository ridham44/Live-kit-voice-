...
// createAgent() now uses LocalLLM (src/llm/localLlm.ts) instead of LiveKit's hosted inference,
// so these evaluations need a real local LLM server running at LOCAL_LLM_URL. Skip with a clear
// message rather than failing confusingly when one isn't available.
const localLlmReachable = await isLocalLlmReachable();
if (!localLlmReachable) {
  console.warn(
    '[test] LOCAL_LLM_URL is not set or not reachable — skipping agent evaluation tests. ' +
      'Start a local OpenAI-compatible LLM server and set LOCAL_LLM_URL in .env.local to run these.',
  );
}

describe.skipIf(!localLlmReachable)('agent evaluation', () => {
